...
import { AFFILIATE_NAV_LINKS, AFFILIATE_PROFILE_LINKS } from '../../constants';
...
export const AffiliateSidebar: React.FC<AffiliateSidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
    const { locale = Locale.English } = useParams<{ locale: Locale }>();
    const trigger = useRef<HTMLButtonElement>(null);
    const sidebar = useRef<HTMLElement>(null);

    const basePath = `/${locale}/affiliate`;
            ...
            <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
                ...
                <nav className="mt-5 py-4 px-4 lg:px-6">
                    <div>
                        ...
                        <NavGroup links={AFFILIATE_NAV_LINKS} basePath={basePath} />
                    </div>

                    <div className="mt-8">
                        <h3 className="mb-4 ml-4 text-sm font-semibold text-gray-400">ACCOUNT</h3>
                        <NavGroup links={AFFILIATE_PROFILE_LINKS} basePath={basePath} />
                    </div>
                </nav>
                {/* <!-- Sidebar Menu --> */}
            </div>
