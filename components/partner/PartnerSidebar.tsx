...
import { PARTNER_NAV_LINKS, PARTNER_PROFILE_LINKS } from '../../constants';
...
export const PartnerSidebar: React.FC<PartnerSidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
    const { locale = Locale.English } = useParams<{ locale: Locale }>();
    const trigger = useRef<HTMLButtonElement>(null);
    const sidebar = useRef<HTMLElement>(null);

    const basePath = `/${locale}/partner`;
            ...
            <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
                ...
                <nav className="mt-5 py-4 px-4 lg:px-6">
                    <div>
                        ...
                        <NavGroup links={PARTNER_NAV_LINKS} basePath={basePath} />
                    </div>

                    <div className="mt-8">
                        <h3 className="mb-4 ml-4 text-sm font-semibold text-gray-400">PROFILE</h3>
                        <NavGroup links={PARTNER_PROFILE_LINKS} basePath={basePath} />
                    </div>
                </nav>
                {/* <!-- Sidebar Menu --> */}
            </div>
