...
import { STUDENT_NAV_LINKS, STUDENT_PROFILE_LINKS } from '../../constants';
...
const NavGroup: React.FC<{ links: { name: string; href: string; icon: keyof typeof Icons }[], basePath: string }> = ({ links, basePath }) => {
    const { t } = useI18n();
    return (
        <ul className="space-y-1.5">
            {links.map((link) => {
                // FIX: Explicitly type the dynamic Icon component to ensure TypeScript recognizes the className prop.
                const Icon: React.FC<{ className?: string }> = Icons[link.icon as keyof typeof Icons] || Icons.HelpCircleIcon;
                return (
                    <li key={link.name}>
                        <NavLink
...
export const StudentSidebar: React.FC<StudentSidebarProps> = ({ sidebarOpen, setSidebarOpen }) => {
    ...
    useEffect(() => {
        const keyHandler = ({ keyCode }: KeyboardEvent) => {
            // FIX: Corrected keycode for ESC key to 27
            ...
            <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
                ...
                <nav className="mt-5 py-4 px-4 lg:px-6">
                    <div>
                        ...
                        <NavGroup links={STUDENT_NAV_LINKS} basePath={basePath} />
                    </div>

                    <div className="mt-8">
                        <h3 className="mb-4 ml-4 text-sm font-semibold text-gray-400">PROFILE</h3>
                        <NavGroup links={STUDENT_PROFILE_LINKS} basePath={basePath} />
                    </div>
                </nav>
                {/* <!-- Sidebar Menu --> */}
            </div>
