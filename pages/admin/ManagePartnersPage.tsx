    ...
    return (
        <div className="space-y-6">
            <ConfirmDeleteModal
                ...
                message={`Tem certeza que deseja excluir o parceiro "${itemToDelete?.name}"?`}
            />
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <h1 className="text-3xl font-bold text-ift-dark-blue">{t('page_title_admin_manage_partners')}</h1>
                <Link to={`/${locale}/admin/manage-partners/new/edit`} className="flex items-center justify-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-ift-light-green rounded-md hover:opacity-90 transition-opacity">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-500">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                <th scope="col" className="px-6 py-3">Nome do Parceiro</th>
                        ...
                        <tbody>
                            {MOCK_PARTNERS.map((partner) => (
                                <tr key={partner.id} className="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4">{partner.members.toLocaleString('pt-BR')}</td>
                                    <td className="px-6 py-4">{getStatusBadge(partner.status)}</td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end items-center gap-4">
                                            <Link to={`/${locale}/admin/manage-partners/${partner.id}/edit`} className="text-blue-600 hover:underline"><EditIcon className="w-5 h-5" /></Link>
                                            <button onClick={() => openDeleteModal(partner)} className="text-red-600 hover:underline"><TrashIcon className="w-5 h-5" /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
