...
import { SearchIcon, EditIcon, TrashIcon, HandshakeIcon } from '../../components/ui/Icons';

export const ManagePartnershipsPage: React.FC = () => {
    const { t } = useI18n();
    ...
    const getStatusBadge = (status: string) => (
        <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
            {t(`partnership_list_status_${status}`)}
        </span>
    );
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <h1 className="text-3xl font-bold text-ift-dark-blue">{t('page_title_admin_manage_partnerships')}</h1>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-500">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                <th scope="col" className="px-6 py-3">Nome do Convênio</th>
                                <th scope="col" className="px-6 py-3">Bolsas</th>
